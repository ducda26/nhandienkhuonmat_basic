...
# Step 01:
imgElon = face_recognition.load_image_file("./pic/thien_01.jpg")
imaElon = cv2.cvtColor(imgElon, cv2.COLOR_BGR2RGB)
imgCheck = face_recognition.load_image_file("./pic/thien_02.jpg")
...
# Chỉnh lại kích thước của ảnh để hiển thị với kích thước cố định
desired_width = 450
imgElon = cv2.resize(imgElon, (desired_width, int(
    desired_width * imgElon.shape[0] / imgElon.shape[1])))
imgCheck = cv2.resize(imgCheck, (desired_width, int(
    desired_width * imgCheck.shape[0] / imgCheck.shape[1])))
